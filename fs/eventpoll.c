...
struct epitem {
	union {
		...
		struct rb_node rbn; // jxh: 指向红黑树的节点
		/* Used to free the struct epitem */
		struct rcu_head rcu;
	};

	/* List header used to link this structure to the eventpoll ready list */
	struct list_head rdllink; // jxh: 事件就绪的fd链表
	...
	struct epoll_filefd ffd; // jxh: 文件fd
	...
	struct eppoll_entry *pwqlist; // jxh: 等待队列

	/* The "container" of this item */
	struct eventpoll *ep; // jxh: 指向eventpoll控制块
	...
	struct epoll_event event; // jxh: 事件
};
...
struct eventpoll {
	...
	wait_queue_head_t wq; // jxh: 调用进程等待队列

	/* Wait queue used by file->poll() */
	wait_queue_head_t poll_wait; // jxh: 等待队列队头（如果被监听的文件是一个epoll类型）

	/* List of ready file descriptors */
	struct list_head rdllist; // jxh: 已经准备好的文件描述符队列队头（双向链表）

	/* Lock which protects rdllist and ovflist */
	rwlock_t lock; // jxh: 读写锁

	/* RB tree root used to store monitored fd structs */
	struct rb_root_cached rbr; // jxh: 红黑树root节点（存储epitem）
	...
	struct user_struct *user; // jxh: 当前进程用户

	struct file *file; // jxh: 真正的文件
...
static int ep_alloc(struct eventpoll **pep)
{
	...
	ep = kzalloc(sizeof(*ep), GFP_KERNEL); // jxh: 申请内存
	if (unlikely(!ep))
		return -ENOMEM;

	mutex_init(&ep->mtx); // jxh: 初始化互斥锁
	rwlock_init(&ep->lock); // jxh: 初始化读写锁
	init_waitqueue_head(&ep->wq); // jxh: 初始化等待队列头
	init_waitqueue_head(&ep->poll_wait);
	INIT_LIST_HEAD(&ep->rdllist); // jxh: /初始化就绪队列
	ep->rbr = RB_ROOT_CACHED; // jxh: 初始化红黑树
	ep->ovflist = EP_UNACTIVE_PTR;
	ep->user = get_current_user(); // jxh: 当前用户
...
static struct epitem *ep_find(struct eventpoll *ep, struct file *file, int fd)
{
	...
	ep_set_ffd(&ffd, file, fd); // jxh: 初始化指针
	// jxh: 二分搜索
 ...
 */
// jxh: 插入操作（必须在持有“mtx”的情况下调用）
static int ep_insert(struct eventpoll *ep, const struct epoll_event *event,
		     struct file *tfile, int fd, int full_check)
{
	int error, pwake = 0;
	__poll_t revents;
	struct epitem *epi; // jxh: 红黑树节点结构体
	struct ep_pqueue epq;
	struct eventpoll *tep = NULL;

	if (is_file_epoll(tfile)) // jxh: 判断是否是epoll文件
		tep = tfile->private_data;

	lockdep_assert_irqs_enabled();

	// jxh: epoll可监控的最大值 unsigned int sysctl_nr_open __read_mostly = 1024*1024;
	...
	// jxh: 从缓存中申请内存,申请失败减掉计数，epi_cache内存池在epoll模块初始化时已经分配,这里根据slab直接取一个epitem
	...
	epi->ep = ep; // jxh: 设置指向ep的指针
	ep_set_ffd(&epi->ffd, tfile, fd); // jxh: 设置ffd
	epi->event = *event; // jxh: 设置监听的事件
	...
	// jxh: 如果events里设置了EPOLLWAKEUP, 还需要为autosleep创建一个唤醒源 ep_create_wakeup_source
	...
	// jxh: 如果当前的epi已经就绪了，上面的revents返回就绪的事件，则把其加入就绪列表rdllink
...
static int do_epoll_create(int flags)
{
	 ...
	 */
	// jxh: 传入应用创建eventpoll的结构体
	error = ep_alloc(&ep);
	if (error < 0) // jxh: 出现错误返回错误
		return error;
	/*
	 * Creates all the items needed to setup an eventpoll file. That is,
	 * a file structure and a free file descriptor.
	 */
	// jxh: 创建设置eventpoll文件所需的所有项。即文件结构和自由文件描述符。
	fd = get_unused_fd_flags(O_RDWR | (flags & O_CLOEXEC));
	if (fd < 0) {
		error = fd;
		goto out_free_ep;
	}
	// jxh: 创建一个名字为“[eventpoll]”的eventpollfs文件描述符
		 ...
		 bool nonblock)
{
	...
	int full_check = 0;
	struct fd f, tf;
	struct eventpoll *ep; // jxh: eventpoll结构体
	struct epitem *epi; // jxh: epitem结构体
	struct eventpoll *tep = NULL;

	error = -EBADF;
	f = fdget(epfd); // jxh: 获取文件描述符
	if (!f.file)
		goto error_return;

	/* Get the "struct file *" for the target file */
	tf = fdget(fd); // jxh: 目标文件
	if (!tf.file)
		goto error_fput;

	/* The target file descriptor must support poll */
	error = -EPERM;
	if (!file_can_poll(tf.file)) // jxh: 如果文件不支持则直接退出
		goto error_tgt_fput;

	/* Check if EPOLLWAKEUP is allowed */
	if (ep_op_has_event(op)) // jxh: 检查是否支持epoll唤醒
	 ...
	 */
	error = -EINVAL;
	if (f.file == tf.file || !is_file_epoll(f.file)) // jxh: 检查文件
		goto error_tgt_fput;
	 ...
	 */
	error = epoll_mutex_lock(&ep->mtx, 0, nonblock); // jxh: 需要获取mutex对红黑树进行操作
	if (error)
		goto error_tgt_fput;
	if (op == EPOLL_CTL_ADD) {
	 ...
	 */
	// jxh: 从红黑树中寻找添加的fd是否存在，存在则返回到ep中，否则返回NULL
	epi = ep_find(ep, tf.file, fd);

	error = -EINVAL;
	switch (op) { // jxh: 区分op类型增删改
	case EPOLL_CTL_ADD: // jxh: 新增
		if (!epi) { // jxh: 查找不到则添加
			epds->events |= EPOLLERR | EPOLLHUP;
			error = ep_insert(ep, epds, tf.file, fd, full_check); // jxh: 插入
		} else // jxh: 否则不重复添加
			error = -EEXIST;
		break;
	case EPOLL_CTL_DEL: // jxh: 删除
		if (epi) {
			/*
			 * The eventpoll itself is still alive: the refcount
			 * can't go to zero here.
			 */
			ep_remove_safe(ep, epi); // jxh: 删除
			error = 0;
		} else {
			error = -ENOENT;
		}
		break;
	case EPOLL_CTL_MOD: // jxh: 修改
		if (epi) {
			if (!(epi->event.events & EPOLLEXCLUSIVE)) {
				epds->events |= EPOLLERR | EPOLLHUP;
 ...
 */
// jxh: epoll系统调用函数 实现插入/删除/修改fd的功能
SYSCALL_DEFINE4(epoll_ctl, int, epfd, int, op, int, fd,
		struct epoll_event __user *, event)
{
	struct epoll_event epds;

	// jxh: 判断是否需要来自用户空间的事件副本 && 拷贝事件到内核空间
